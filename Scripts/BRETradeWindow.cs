...
{
    /// <summary>
    /// Implements Better Random Encounter's Trade Window.
    /// </summary>
    public class BRETradeWindow : DaggerfallInventoryWindow, IMacroContextProvider // Tomorrow, work on creating custom trade window to use for modded trade window use for events, etc.
    {
        ...
        public BRETradeWindow(IUserInterfaceManager uiManager, DaggerfallBaseWindow previous = null, WindowModes windowMode = WindowModes.Sell, IGuild guild = null)
        ...
        private void DoSteal()
        {
            ...
            {
                int chanceBeingDetected = 130 - GameManager.Instance.PlayerEntity.Skills.GetLiveSkillValue(DFCareer.Skills.Pickpocket);
                PlayerEntity.TallySkill(DFCareer.Skills.Pickpocket, 3);

                if (basketItems.Count <= 1 && Dice100.FailedRoll(chanceBeingDetected))
                {
                    DaggerfallUI.AddHUDText("With a slight of hand, you pocket the potion without the alchemist noticing.", 2);
                    RaiseOnTradeHandler(1, 0);
                    PlayerEntity.Items.TransferAll(basketItems); // Tomorrow make the changes to the stealing, as well as the suggested punishments/traps for opening inventory of murdered merchants, etc.
                    PlayerEntity.TallyCrimeGuildRequirements(true, 1);
                }
                else
                {   // Turn merchant agressive.
                    DaggerfallUI.AddHUDText("Damn thief!", 2);
                    RaiseOnTradeHandler(0, 0);
                    Game.GameManager.Instance.MakeEnemiesHostile();
                }
                CloseWindow(); // This does close the entire trade window when using the steal button, success and fails alike.
            }
        }
        ...
        {
            private BRETradeWindow parent;
            public BRETradeMacroDataSource(BRETradeWindow tradeWindow)
            {
